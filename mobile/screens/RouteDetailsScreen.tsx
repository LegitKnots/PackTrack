...
import StartIcon from '../src/img/icons/start.svg'
import EndIcon from '../src/img/icons/end.svg'
      ...
      >
        {routeData.waypoints?.map((wp: any, idx: number) => {
          ...
          if (idx === 0) {
            icon = 'flag'
            color = 'lime'
          } else if (idx === routeData.waypoints.length - 1) {
            icon = 'flag-checkered'
            color = 'red'
          }

          return (
            <Marker coordinate={{ latitude: wp.lat, longitude: wp.lng }}>
              {idx === 0 ? (
