...
export default function PackDetailsScreen() {
        ...
        onPress: async () => {
          try {
            ...
            const res = await fetch(
              `${SERVER_URI}/api/packs/${pack.id}/leave`,
              {
                method: 'POST',
                headers: {
                  Authorization: `Bearer ${token}`,
  ...
  const searchUsers = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${SERVER_URI}/api/users/search?q=${encodeURIComponent(searchQuery)}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
      ...
      const filteredResults = data.filter(
        (user: any) => !pack.members.includes(user.id),
      );
  ...
  const searchRoutes = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${SERVER_URI}/api/routes/search?q=${encodeURIComponent(searchQuery)}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
      ...
      const filteredResults = data.filter(
        (route: RouteType) =>
          !packRouteIds.includes(route.id) && !packRouteIds.includes(route._id),
      );
    ...
    try {
      ...
      setSearchResults(
        searchResults.filter(
          route => route.id !== routeId && route._id !== routeId,
        ),
      );
    } catch (err: any) {
  ...
  const renderRouteCard = (route: RouteType) => (
    <TouchableOpacity
      ...
      <Text style={styles.routeDetail} numberOfLines={1}>
        ...
        {route.waypoints?.[route.waypoints.length - 1]?.label || 'Unknown'}
      </Text>
      ...
      <View style={styles.shareModalContainer}>
        ...
        <View style={styles.linkContainer}>
          ...
          <Text style={styles.shareInstructions}>
            Share this QR code with others to let them join this pack.
          </Text>
        </View>
      </View>
    </Modal>
      ...
      <View style={styles.modalContainer}>
        ...
        {isSearching ? (
          ...
          <View style={styles.emptyResults}>
            ...
            <Text style={styles.emptyResultsSubtext}>
              Find users by name or username to invite them to your pack
            </Text>
          </View>
        )}
      </View>
      ...
      <View style={styles.modalContainer}>
        ...
        {isSearching ? (
              ...
              <View style={styles.routeItem}>
                <View style={styles.routeInfo}>
                  ...
                  <Text style={styles.routeItemDetail} numberOfLines={1}>
                    ...
                    {item.waypoints?.[item.waypoints.length - 1]?.label ||
                      'Unknown'}
                  </Text>
                </View>
                <TouchableOpacity
                  style={styles.addButton}
                  onPress={() => addRouteToPack(item.id || item._id)}>
          ...
          <View style={styles.emptyResults}>
            ...
            <Text style={styles.emptyResultsSubtext}>
              Find routes to add to your pack
            </Text>
          </View>
        )}
      </View>
      ...
      <ScrollView
        ...
        <View style={styles.packImageContainer}>
          {pack.imageUrl ? (
            ...
            <View style={[styles.packImage, styles.packImagePlaceholder]}>
              <Text style={styles.packImageText}>
        ...
        <View style={styles.actionButtons}>
          <TouchableOpacity
            ...
            onPress={() =>
              navigation.navigate('PackMembers', {packId: pack.id})
            }>
          ...
          {pack.options?.hasChat && (
            <TouchableOpacity
              style={styles.actionButton}
              onPress={() =>
                navigation.navigate('PackChat', {packId: pack.id})
              }>
          ...
          {isOwner && (
            <TouchableOpacity
              style={styles.actionButton}
              onPress={() => navigation.navigate('PackSettings', {pack})}>
        ...
        <View style={styles.routesSection}>
          <View style={styles.sectionHeader}>
            ...
            {isMember && (
              <TouchableOpacity
                ...
                onPress={() => setAddRouteModalVisible(true)}>
          ...
          {loadingRoutes ? (
            ...
            <View style={styles.emptyRoutes}>
              ...
              {isMember && (
                <Text style={styles.emptyRoutesSubtext}>
                  Add routes to this pack to collaborate with other members
                </Text>
              )}
            </View>
          )}
